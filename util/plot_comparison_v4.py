...
# RCAP 데이터 (Bq*s/m3) - Briggs 모델, 첫 번째 값 제외, 10km까지만 (B, C, D, E만)
rcap = {
...
# Gpuff 데이터 (Bq*s/m3) - Briggs 모델, 첫 번째 값 제외, 10km까지만 (B, C, D, E만)
gpuff = {
...
for idx, sc in enumerate(stability_classes):
    ...
    # RCAP 데이터 (실선)
    ax.loglog(distances, rcap[sc], 'o-', color=colors[sc], label='RCAP (Briggs)', linewidth=2, markersize=5)
...
plt.suptitle('RCAP vs Gpuff: Briggs-McElroy-Pooler Dispersion Model', fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig('X:/code/gpuffv4/gpuff-RCAPv3/util/comparison_plot_v4.png', dpi=150, bbox_inches='tight')
print("Plot 1 saved to: X:/code/gpuffv4/gpuff-RCAPv3/util/comparison_plot_v4.png")
...
ax2.set_title('RCAP vs Gpuff: Briggs-McElroy-Pooler Dispersion Model', fontsize=14, fontweight='bold')
ax2.legend(loc='upper right', fontsize=9, ncol=2)
ax2.grid(True, which='both', alpha=0.3)
ax2.set_xlim([0.15, 12])

plt.tight_layout()
plt.savefig('X:/code/gpuffv4/gpuff-RCAPv3/util/comparison_plot_all_v4.png', dpi=150, bbox_inches='tight')
print("Plot 2 saved to: X:/code/gpuffv4/gpuff-RCAPv3/util/comparison_plot_all_v4.png")
